...
// ฟังก์ชันย่อ htmlspecialchars (ไว้ใช้ป้องกัน XSS ตอนแสดงผล)
function h($string)
{
...
// รับค่า HN
$hn = isset($_GET['hn']) ? $_GET['hn'] : '6603321';
...
try {
    ...
    if (!$patient) {
        die("<div style='text-align:center; margin-top:50px; color:red;'>
                <h3>ไม่พบข้อมูลผู้ป่วย HN: " . h($hn) . "</h3>
             </div>");
    }
...
<head>
    ...
    <title>ระบบประเมินภาวะโภชนาการ | โรงพยาบาลกำแพงเพชร</title>
...
<body>
    ...
    <div class="main-content" style="padding: 0 20px 40px 20px; max-width: 1200px; margin: 0 auto;">
                    ...
                    <table class="screening-table">
                        ...
                        <tbody>
                            ...
                            <tr class="screening-row">
                                ...
                                <td class="text-center"><input type="radio" name="q3" class="table-radio screen-input"
                                        value="yes"></td>
                                <td class="text-center"><input type="radio" name="q3" class="table-radio screen-input"
                                        value="no" checked></td>
                            </tr>
        ...
        <div id="assessmentWrapper">
            <div class="assessment-section-title">แบบประเมินภาวะโภชนาการ (Nutrition Alert Form)</div>
    ...
    <script>
        ...
        function toggleSourceInput() {
            const isOther = document.getElementById('sourceOther').checked;
            const otherInput = document.getElementById('sourceOtherText');
            if (isOther) {
        ...
        function toggleDiseaseInput(checkboxId, inputId) {
        ...
        function initRealTimeCalculation() {
            // Attach event listeners to all inputs affecting the score
            const allInputs = document.querySelectorAll('#assessmentForm input');
            allInputs.forEach(input => {
                input.addEventListener('change', updateRealTimeScore);
                input.addEventListener('input', updateRealTimeScore); // For text/number inputs
            });
        }

        function calculateBMI() {
            // For Screening section
            ...
            if (weight > 0 && heightCm > 0) {
                ...
                if (bmi < 18.5 || bmi >= 25.0) q3Yes.checked = true;
                else q3No.checked = true;
            } else {
                bmiInput.value = '';
            }
        }
        ...
        function calcAssessBMI() {
            // For Assessment section - also triggers score update
            ...
            // Trigger update after BMI change
            updateRealTimeScore();
        }
        ...
        function getScore() {
            ...
            // 2. Radio Groups (Body, Weight, Food, Access)
            ['bodyShape', 'weightChange', 'foodType', 'foodQty', 'foodAccess'].forEach(name => {
                const checked = document.querySelector(`input[name="${name}"]:checked`);
                if (checked) totalScore += parseInt(checked.value) || 0;
            });

            // 3. Symptoms (Checkboxes)
            document.querySelectorAll('input[name^="symptom_"][type="checkbox"]:checked').forEach(cb => {
                totalScore += parseInt(cb.value) || 0;
            });

            // 4. Diseases (Score per item: Moderate = 3, Severe = 6)
            const mod = document.querySelectorAll('input[name="disease_moderate"]:checked, input[name="disease_moderate_other"]:checked');
            totalScore += mod.length * 3; // +3 points for each moderate disease selected

            const sev = document.querySelectorAll('input[name="disease_severe"]:checked, input[name="disease_severe_other"]:checked');
            totalScore += sev.length * 6; // +6 points for each severe disease selected
        ...
        function updateRealTimeScore() {
            ...
            const scoreCard = document.getElementById('scoreCardBox');
            const riskCard = document.getElementById('riskCardBox');
            const riskLabel = document.getElementById('realTimeRiskLabel');
            const riskGroup = document.getElementById('realTimeRiskGroup');
            const riskDetail = document.getElementById('realTimeRiskDetail');

            // Reset classes
            const classes = ['risk-color-low', 'risk-color-med', 'risk-color-high', 'risk-color-neutral'];
            scoreCard.classList.remove(...classes);
            riskCard.classList.remove(...classes);

            if (score <= 5) {
                // Low Risk
                scoreCard.classList.add('risk-color-low');
                riskCard.classList.add('risk-color-low');
                riskLabel.innerText = "NAF A";
                riskGroup.innerText = "Normal-Mild mulnutrition";
                riskDetail.innerText = "ไม่พบความเสี่ยงต่อการเกิดภาวะทุพโภชนาการ พยาบาลจะทำหน้าที่ประเมินภาวะโภชนาการซ้ำภายใน 7 วัน";
            } else if (score <= 10) {
                // Moderate Risk
                scoreCard.classList.add('risk-color-med');
                riskCard.classList.add('risk-color-med');
                riskLabel.innerText = "NAF B";
                riskGroup.innerText = "Moderate mulnutrition";
                riskDetail.innerText = "กรุณาแจ้งให้แพทย์และนักกำหนดอาหาร/นักโภชนาการทราบผลทันทีที่มีภาวะทุพโภชนาการ ให้นักกำหนดอาหาร/นักโภชนาการทำการประเมินภาวะโภชนาการ และให้แพทย์ทำการดูแลรักษาภายใน 3 วัน";
            } else {
                // High Risk
                scoreCard.classList.add('risk-color-high');
                riskCard.classList.add('risk-color-high');
                riskLabel.innerText = "NAF C";
                riskGroup.innerText = "Severe mulnutrition";
                riskDetail.innerText = "กรุณาแจ้งให้แพทย์และนักกำหนดอาหาร/นักโภชนาการทราบผลทันทีที่มีภาวะทุพโภชนาการ ให้นักกำหนดอาหาร/นักโภชนาการทำการประเมินภาวะโภชนาการ และให้แพทย์ทำการดูแลรักษาภายใน 24 ชั่วโมง";
            }
        }

        function saveScreening() {
            // ... (Logic เดิม) ...
            if (!confirm('ยืนยันการบันทึกผลคัดกรอง?')) return;
            document.getElementById('saveScreeningRow').style.display = 'none';

            let yesCount = 0;
            for (let i = 1; i <= 4; i++) {
                const radio = document.querySelector(`input[name="q${i}"][value="yes"]`);
                if (radio && radio.checked) yesCount++;
            }

            const resultBox = document.getElementById('resultBox');
            const resultTitle = document.getElementById('resultTitle');
            const resultDesc = document.getElementById('resultDesc');
            const highRiskOptions = document.getElementById('highRiskOptions');

            resultBox.classList.remove('show', 'risk-low', 'risk-high');
            resultBox.style.display = 'block';

            if (yesCount >= 2) {
                resultBox.classList.add('show', 'risk-high');
                resultTitle.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> พบความเสี่ยง (High Risk)';
                resultDesc.innerText = 'ผลคัดกรองถูกบันทึกแล้ว ผู้ป่วยมีความเสี่ยงต้องประเมินต่อ';
                highRiskOptions.style.display = 'block';
            } else {
                resultBox.classList.add('show', 'risk-low');
                resultTitle.innerHTML = '<i class="fa-solid fa-check-circle"></i> ความเสี่ยงต่ำ (Low Risk)';
                resultDesc.innerText = 'บันทึกข้อมูลเรียบร้อยแล้ว ให้คัดกรองซ้ำสัปดาห์ละ 1 ครั้ง';
                alert('บันทึกผลคัดกรองเรียบร้อย (ความเสี่ยงต่ำ)');
            }
        }

        function saveLater() {
            alert('ระบบได้บันทึกสถานะ "รอการประเมิน" ไว้แล้ว');
            window.location.href = 'patient_profile.php';
        }

        function openAssessment() {
            document.getElementById('highRiskOptions').style.display = 'none';
            const assessmentWrapper = document.getElementById('assessmentWrapper');
            assessmentWrapper.style.display = 'block';
            assessmentWrapper.scrollIntoView({
                behavior: 'smooth'
            });
            // Init calculation on open
            updateRealTimeScore();
        }
        ...
        function saveAssessment() {
            const score = getScore();
            let risk = "";
            if (score <= 5) risk = "NAF A (Normal/Mild)";
            else if (score <= 10) risk = "NAF B (Moderate)";
            else risk = "NAF C (Severe)";

            if (!confirm(`คะแนนรวม: ${score}\nระดับความเสี่ยง: ${risk}\n\nยืนยันการบันทึกข้อมูล?`)) return;
            alert('บันทึกแบบประเมินเรียบร้อย');
            window.location.href = 'patient_profile.php';
        }
    </script>
</body>
